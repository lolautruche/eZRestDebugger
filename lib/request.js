...
RequestHandler.prototype = {
  ...
  send: function() 
  {
    ...
    {
      var postParams = {};
      var aParams = $('#postParams').val().split('&');
      aParams.forEach(function(val) {
        var aCurrentParam = val.split('=');
        postParams[aCurrentParam[0]] = aCurrentParam[1];
      });
      
      requestParams.data = postParams;
    }
  ...
  success: function(data, textStatus, jqXHR ) 
  {
    ...
    var responseText = beautify.js(jqXHR.responseText);
  ...
  var ajaxParams = {
    error: function(jqXHR, textStatus, errorThrown) {
      switch(jqXHR.status)
      {
        case HTTP_UNAUTHORIZED: // 401
          inst.parent.error.call(inst, jqXHR, textStatus, errorThrown);
          if(inst.authType === REST_AUTHTYPE_BASIC)
