...
OAuth2RequestHandler.prototype.send = function() {
  ...
  else
  {
    ...
    var url = $('#restEndpointServer').val() + OAUTH2_AUTHORIZE_URI + '?redirect_uri='+encodeURIComponent(clientEndpoint)+'&client_id='+encodeURIComponent(clientId)+'&response_type=token';
    ...
    newHTMLLoader.addEventListener('locationChange', function(evt) {
      ...
      if(currentLocation.indexOf('error') != -1) {
      ...
      else if (currentLocation.indexOf('access_token') != -1) { // We're expecting OAuth access_token to be in the URL
        ...
        $('#oauth2AccessToken').val(oauthParams.access_token);
        $('#oauth2RefreshToken').val(oauthParams.refresh_token);
        if(rest.remember)
          rest.storeSettings();
